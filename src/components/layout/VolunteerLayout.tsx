...
export interface VolunteerLayoutProps {
  activeNavId?: string;
...
export const VolunteerLayout: React.FC<VolunteerLayoutProps> = ({
  activeNavId = 'vol-dash',
  onNavigate,
  title,
  subtitle,
  actions,
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xs font-bold text-slate-900">Volunteer Hub</h2>
              <p className="text-[10px] text-amber-600 font-medium">Community Service</p>
            </div>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {NAVIGATION_CONFIG.volunteer.map((item) => {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between gap-4 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-slate-800">Volunteer Portal</span>
          </div>

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <Avatar fallback={user?.name || 'Volunteer'} size="xs" />
